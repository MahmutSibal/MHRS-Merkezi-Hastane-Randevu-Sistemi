...
public sealed class ReportRepository : IReportRepository
{
    ...
    public async Task<IReadOnlyList<TopDoctorDto>> GetTopDoctorsLastDaysAsync(int days, int take, int? hospitalId, CancellationToken ct)
    {
        if (days <= 0) days = 30;
        if (take <= 0) take = 10;
        ...
        if (hospitalId is not null)
        {
            baseQuery = baseQuery.Where(a =>
                ...
                a.Doctor.Department.HospitalId == hospitalId.Value);
        }
    ...
    public async Task<AppointmentSummaryDto> GetAppointmentSummaryAsync(int days, int? hospitalId, CancellationToken ct)
    {
        if (days <= 0) days = 30;
