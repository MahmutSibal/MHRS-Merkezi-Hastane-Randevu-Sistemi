...
export function NotificationCenter() {
  ...
  const unreadCount = useMemo(() => items.filter((x) => !x.isSent).length, [items]);
              ...
              {items.map((item) => (
                <div key={item.id} className="rounded-xl border border-slate-200 p-3 text-xs dark:border-slate-700">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {!item.isSent ? <span className="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] text-blue-700">Yeni</span> : null}
                  </div>
